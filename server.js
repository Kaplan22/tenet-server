...
// v2.1 · WebSocket transport layer
//
...
// Volume-tiered: 10bps (≤$10M) · 7bps ($10M–$100M) · 5bps ($100M–$250M) · negotiated ($250M+)
// Tokenized RWAs: 1.5bps flat (all sizes) — set isRWA flag on intent when applicable
function calcFeeBps(usdcAmount) {
  if (usdcAmount < 1_000_000)    return 15;
  if (usdcAmount < 10_000_000)   return 10;
  if (usdcAmount < 100_000_000)  return 7;
  if (usdcAmount < 250_000_000)  return 5;
  return 3; // negotiated — $250M+ placeholder
}
function calcFee(usdcAmount, isRWA=false) {
  if (isRWA) return usdcAmount * 0.00015; // 1.5bps flat for tokenized RWAs
  return usdcAmount * (calcFeeBps(usdcAmount) / 10000);
}
function feeTierLabel(usdcAmount, isRWA=false) {
  if (isRWA) return '1.5bps · tokenized RWA flat';
  if (usdcAmount < 1_000_000)    return '15bps · < $1M tier';
  if (usdcAmount < 10_000_000)   return '10bps · $1M–$10M tier';
  if (usdcAmount < 100_000_000)  return '7bps · $10M–$100M tier';
  if (usdcAmount < 250_000_000)  return '5bps · $100M–$250M tier';
  return 'negotiated · $250M+ tier';
}
...
class OrchestrationEngine {
  ...
  async submitIntent(intent) {
    ...
    const isRWA      = intent.isRWA || false;
    const fee        = calcFee(usdcAmount, isRWA);
    const feeBps     = isRWA ? 1.5 : calcFeeBps(usdcAmount);
    ...
    const order = {
      ...
      asset:asset.toUpperCase(), usdcAmount, fee, feeBps, feeTier:feeTierLabel(usdcAmount, isRWA), total, isRWA,
...
// HTTP server for Railway health checks and WSS upgrade
const httpServer = http.createServer((req, res) => {
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({
    status: 'ok',
    service: 'Tenet Systems — Orchestration Engine',
    version: '2.1',
...
httpServer.listen(PORT, () => {
  console.log('[' + new Date().toISOString().slice(11,19) + '] HTTP + WebSocket listening on port ' + PORT);
});
